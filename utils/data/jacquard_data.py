...
class JacquardDataset(GraspDatasetBase):
    ...
    def __init__(self, file_path, start=0.0, end=1.0, ds_rotate=0, **kwargs):
        ...
        graspf = glob.glob(os.path.join(file_path, '*', '*cpos.txt'))
        graspf.sort()
        l = len(graspf)
        random.shuffle(graspf)    #if used ,that is IW , otherwise OW
        ...
        depthf = [f.replace('cpos.txt', 'd.png') for f in graspf]
        rgbf = [f.replace('d.png', '.png') for f in depthf]
