...
class GraspRectangle:
    ...
    def g_iou(self, gr, angle_threshold=np.pi/6):
        """
    ...
    def plot(self, ax, color=None):
        ...
        # ax.plot(points[:, 1], points[:, 0], color=color, alpha=0.5)

        point_width1 = np.vstack((self.points[0], self.points[1]))
        point_width2 = np.vstack((self.points[2], self.points[3]))
        point_height1 = np.vstack((self.points[1], self.points[2]))
        point_height2 = np.vstack((self.points[3], self.points[0]))
        ax.plot(point_width1[:, 1], point_width1[:, 0], "c--")
        ax.plot(point_height1[:, 1], point_height1[:, 0], "r-", linewidth=4)
        ax.plot(point_width2[:, 1], point_width2[:, 0], "c--")
        ax.plot(point_height2[:, 1], point_height2[:, 0], "r-", linewidth=4)
...
class GraspHgBoundBox:
    """
    Representation of a grasp in the common "Grasp hourglass frame" format.
    """
    def __init__(self, points):
        self.points = points
    ...
    def as_grasp(self):
        """
        :return: GraspHgBoundBox converted to a Grasp
        """
        return Grasp(self.center, self.angle, self.length, self.width)
    ...
    def center(self):
        """
        :return: hourglass frame center point
        """
        return self.points.mean(axis=0).astype(np.int)
    ...
    def length(self):
        """
        :return: hourglass frame length (i.e. along the axis of the grasp)
        """
        dx = self.points[1, 1] - self.points[0, 1]
        dy = self.points[1, 0] - self.points[0, 0]
        return np.sqrt(dx ** 2 + dy ** 2)
    ...
    def width(self):
        """
        :return: hourglass frame width (i.e. perpendicular to the axis of the grasp)
        """
        dy = self.points[2, 1] - self.points[1, 1]
        dx = self.points[2, 0] - self.points[1, 0]
        return np.sqrt(dx ** 2 + dy ** 2)
    ...
    def polygon_coords(self, shape=None):
        """
        :param shape: Output Shape
        :return: Indices of pixels within the grasp hourglass frame polygon.
        """
        center = self.points.mean(axis=0).astype(np.int)
        points = np.vstack((self.points[0], center, self.points[[1, 2]], center, self.points[3]))
    ...
    def iou(self, gr, angle_threshold=np.pi/6):
        ...
        :param gr: GraspingRectangle to compare
        :param angle_threshold: Maximum angle difference between GraspRectangles
        :return: IoU with Grasp Rectangles, Minimum convex closed box containing ground truth box and hourglass                    box, the union of ground truth box and hourglass box
        """
        if abs((self.angle - gr.angle + np.pi/2) % np.pi - np.pi/2) > angle_threshold:
            return 0, 0.5, 1

        rr1, cc1 = self.polygon_coords()     #hourglass box
        rr2, cc2 = polygon(gr.points[:, 0], gr.points[:, 1])    #ground truth rectangle box
    ...
    def copy(self):
        ...
        :return: Copy of self.
        """
        return GraspHgBoundBox(self.points.copy())

    def offset(self, offset):
        """
        Offset grasp hourglass frame
        :param offset: array [y, x] distance to offset
        """
        self.points += np.array(offset).reshape((1, 2))
    ...
    def scale(self, factor):
        """
        :param factor: Scale grasp hourglass frame by factor
        """
        if factor == 1.0:
            return
        self.points *= factor
    ...
    def plot(self, ax, color=None):
        """
        Plot grasping hourglass frame.
        :param ax: Existing matplotlib axis
        :param color: matplotlib color code (optional)
        """
        center = self.center()
        points = np.vstack((self.points[0], center, self.points[[1, 2]], center, self.points[3], self.points[0]))
        ax.plot(points[:, 1], points[:, 0], "ro--")
        #ax.plot(points[:, 1], points[:, 0], color=color)

    def zoom(self, factor, center):
        """
        Zoom grasp hourglass frame by given factor.
        :param factor: Zoom factor
        :param center: Zoom zenter (focus point, e.g. image center)
        """
        T = np.array(
...
class Grasp:
    ...
    def as_hg(self):
        """
            Convert to GraspHgBoundBox
            :return: GraspHgBoundBox representation of grasp.
            """
        xo = np.cos(self.angle)
        yo = np.sin(self.angle)
        ...
        return GraspHgBoundBox(np.array(
            [
                [y1 - self.width / 2 * xo, x1 - self.width / 2 * yo],
                [y2 - self.width / 2 * xo, x2 - self.width / 2 * yo],
                [y2 + self.width / 2 * xo, x2 + self.width / 2 * yo],
    ...
    def max_giou(self, grs):
        ...
        self_gr = self.as_gr
        max_iou = 0
        for gr in grs:     #generally it is the ground truth bounding box
            iou = self_gr.g_iou(gr)
    ...
    def max_iou(self, grs):
        ...
        enclose_area_res = 0
        for gr in grs:
            iou, union, enclose_area = self_gr.iou(gr)
            if iou > max_iou:
                index += 1
                union_res = union
                enclose_area_res = enclose_area
                max_iou = iou
            if index == 0:
                union_res = union
                enclose_area_res = enclose_area
        return max_iou, union_res, enclose_area_res

    def hourglass_max_iou(self, grs):
        """
        Return maximum IoU between self and a list of GraspRectangles
        :param grs: List of GraspRectangles
        :return: Maximum IoU with any of the GraspRectangles
        ...
        enclose_area_res = 0
        for gr in grs:
            iou, union, enclose_area = self_hg.iou(gr)
            if iou > max_iou:
                index += 1
                union_res = union
                enclose_area_res = enclose_area
...
def detect_grasps(q_img, ang_img, width_img=None, no_grasps=1):
    ...
    """
    local_max = peak_local_max(q_img, min_distance=15, threshold_abs=0.2, num_peaks=no_grasps)  #0.2
